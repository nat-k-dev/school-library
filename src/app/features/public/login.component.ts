  ...
  template: `
    <div class="min-h-screen bg-[#f4f7fb] text-[#14305c] flex flex-col items-center px-5 py-10">
      ...
      <form class="card w-full max-w-sm flex flex-col gap-3" [formGroup]="form" (ngSubmit)="submit()">
        <h1 class="screen-title mb-2">{{ t.auth.loginTitle }}</h1>
        ...
        <button mat-flat-button type="submit" class="primary-button" [disabled]="busy()">{{ t.auth.loginButton }}</button>
        <button mat-button type="button" (click)="forgot()" [disabled]="busy()">{{ t.auth.forgot }}</button>
        ...
        <p class="m-0 mt-3 text-sm text-center text-slate-600">
          {{ t.auth.noAccount }} <a routerLink="/registreren" class="text-[#1f4e9c]">{{ t.auth.toRegister }}</a>
        </p>
      </form>
    </div>
  `,
...
export class LoginComponent {
  ...
  /** Where to go after login; set by the auth guard. */
  ...
  protected async forgot(): Promise<void> {
    ...
    try {
      ...
      this.snackBar.success(T.auth.resetSent);
    } catch (err) {
  ...
  private async run(action: () => Promise<unknown>): Promise<void> {
    ...
    try {
      await action();
      await this.router.navigateByUrl(this.next() ?? '/app');
    } catch (err) {
      this.error.set(describeAuthError(err));
    } finally {
      this.busy.set(false);
